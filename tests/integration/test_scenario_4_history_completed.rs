...
async fn cleanup_task(redis: &RedisService, task_id: &str) {
    ...
    let _: Result<(), redis::RedisError> = conn.del(format!("crawl:task:{}", task_id)).await;
    let _: Result<(), redis::RedisError> = conn.del(format!("crawl:posts:{}", task_id)).await;
    let _: Result<(), redis::RedisError> = conn.del(format!("crawl:post_ids:{}", task_id)).await;
    let _: Result<(), redis::RedisError> =
        conn.del(format!("crawl:checkpoint:{}", task_id)).await;
}
...
impl MockCrawlEngine {
    ...
    async fn save_post(&self, task_id: &str, post: &WeiboPost) -> Result<(), String> {
        ...
        let exists: bool = conn
            .sismember(&post_ids_key, &post.id)
            .await
            .map_err(|e| e.to_string())?;
        ...
        let score = post.created_at.timestamp() as f64;

        let _: () = conn
            .zadd(&posts_key, post_json, score)
            .await
            .map_err(|e| e.to_string())?;
...
mod tests {
    ...
    async fn test_scenario_4_history_completed() {
        let engine = MockCrawlEngine::new().await;
        ...
        // 清理测试数据
        cleanup_task(&engine.redis, &task.id).await;
    ...
    async fn test_post_deduplication() {
        let engine = MockCrawlEngine::new().await;
        let task_id = format!("test_task_{}", Utc::now().timestamp());
        ...
        // 清理测试数据
        cleanup_task(&engine.redis, &task_id).await;
    }
