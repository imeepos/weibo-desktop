...
async fn test_redis_memory_usage() {
    ...
    println!(
        "初始内存使用: {:.2} MB",
        used_memory_before / 1024.0 / 1024.0
    );
    ...
    println!(
        "\n写入后内存使用: {:.2} MB",
        used_memory_after / 1024.0 / 1024.0
    );
    println!("内存增长: {:.2} MB", memory_increase / 1024.0 / 1024.0);
    println!(
        "平均每条帖子: {:.2} bytes",
        memory_increase as f64 / total_count as f64
    );
...
async fn test_pipeline_optimization() {
    ...
    redis
        .save_posts(task_id_pipeline, &posts)
        .await
        .expect("Pipeline写入失败");
    ...
    let mut conn = redis.get_connection().await.expect("获取连接失败");
    for post in posts.iter().take(1000) {
        // 只测试1000条避免太慢
    ...
    // 计算性能提升倍数
    let speedup =
        (single_duration.as_millis() as f64) / (pipeline_duration.as_millis() as f64 * 0.1); // pipeline测试了10倍数据
    ...
    // 清理
    let _: () = conn
        .del(format!("crawl:posts:{}", task_id_pipeline))
        .await
        .unwrap();
    let _: () = conn
        .del(format!("crawl:post_ids:{}", task_id_pipeline))
        .await
        .unwrap();
    let _: () = conn
        .del(format!("crawl:posts:{}", task_id_single))
        .await
        .unwrap();
    let _: () = conn
        .del(format!("crawl:post_ids:{}", task_id_single))
        .await
        .unwrap();
}
